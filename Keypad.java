...
public class Keypad implements ActionListener {
   ...
   InterfaceName method;// The converter object that maps keys to letters
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      String oldText = display.getText();//Creates the local variable for oldText and gets the text on the display
      String newText = oldText;//With the try-catch block, it was necessary to initialize newText above the try-catch block. With the current setup, if the try does detect our exception, then it will simply return the oldText
      try{//Try block to look for our exception
    	  newText = method.append(oldText, i);//Original code calling append
      }
      catch(IllegalArgumentException IllegalArgumentException){//Catch block looking for our exception
      }//Catch block basically does nothing
      display.setText(newText);//Display the text
    ...
    */
   public Keypad(InterfaceName method, String title) {
      this.method = method;
      this.title = title;
